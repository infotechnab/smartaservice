...
<div id="cart_detail">
    <?php echo form_open('payment/do_payment'); ?>
...
<?php if ($this->cart->contents()) { ?>
            ...
            <table width='100%' cellpadding='15' class="cart">
                ...
                <?php if ($cart = $this->cart->contents()) { ?>
                    ...
                    foreach ($cart as $item) {
                        ...
                        <tr class='forTopBorder'>
                            <td>
                                <img src="<?php echo base_url() . 'content/images/' . $item['image1']; ?>" height="50" width="50">
                            </td>
                            <td>
            <?php echo $item['name']; ?> 
                            </td>
                            <td>
                                <input type="text" value="<?php echo $item['qty'] ?>" id="update_qty" size="3" name="item_qnt_<?php echo $item['id']; ?>" class="updateQuantity"> 
                                <input type="hidden" value="<?php echo $item['rowid']; ?>" name="item_row_<?php echo $item['id']; ?>">
                            </td>
                            <td>
            <?php get_currency($item['price']); ?>
                            </td>
                            <td class="sub_total_price">

                            </td>
                            <td>
                                <a href="<?php echo base_url(); ?>index.php/cartDetails/remove/<?php echo $item['rowid']; ?>"><div id="closeSymbol">X</div></a>
                            </td>


                            <?php
                            echo '<input type="hidden" name="item_name[' . $cart_items . ']" value="' . $item['name'] . '" />';
                            echo '<input type="hidden" name="item_code[' . $cart_items . ']" value="' . $item['id'] . '" />';
                            echo '<input type="hidden" name="item_desc[' . $cart_items . ']" value="' . $item['desc'] . '" />';
                            echo '<input type="hidden" name="item_qty[' . $cart_items . ']" value="' . $item['qty'] . '" />';
                            ?>
                        </tr>

                        <?php
                        //Increasing the cart item number 
                        $cart_items++;
                    }
                }
                ?>
                <tr class='forTopBorder'>
                    <td><input type="button" class="updateBtnStyle" value="Update Cart"></td>
                    <td></td>
                    <td></td>
                    <td><b>Total</b>:</td>
    ...
    <div id="order_summary">
        ...
        <div id="order_checkout"  class="updateBtnStyle">
            <input type="submit" class="updateBtnStyle" value="Pay Now">
